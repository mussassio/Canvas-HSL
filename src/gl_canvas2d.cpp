...
void lineComCor( int x1, int y1, int x2, int y2 , Cor c1)
{
    glColor3f(c1.get_r(),c1.get_g(), c1.get_b());
   glBegin(GL_LINES);
      glVertex2d(x1, y1);
      glVertex2d(x2, y2);
   glEnd();
...
void display (void)
{
   ...
   //cria uma projecao ortografica com z entre (-1, 1).
   //parametros: left, right, bottom, top, ou seja, (0,0) --> (largura,altura)
   gluOrtho2D (0.0, (GLdouble) largura, 0.0, (GLdouble) altura);
   glMatrixMode(GL_MODELVIEW);
